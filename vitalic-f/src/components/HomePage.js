...
const getChartData = (topCategories, withdrawTotal) => {
  ...
  const backgroundColors = labels.map((label, index) =>
    index < topCategories.length
      ? `hsla(36, 100%, ${
          100 - (topCategories[index].amount / withdrawTotal) * 100
        }%, 1)`
      : "#999"
  );
...
const HomePage = () => {
  ...
  const fetchSummaryData = async () => {
    ...
    try {
      ...
      const { monthly_top3_summary, other_categories } = response.data;

      const { deposit_total, withdraw_total, top_categories } =
