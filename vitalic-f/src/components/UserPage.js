	...
	const handleSetGoal = async () => {
		try {
			...
			const [response1, response2] = await Promise.race([
				...
				axios.post(
					"http://172.16.13.116:8080/user/updateBudget",
					{
						dailyBudget: dayGoal,
						monthlyBudget: monthGoal,
						weeklyBudget: weekGoal,
	...
	const handleLogout = async () => {
		try {
			...
			const [response1, response2] = await Promise.race([
				...
				axios.post(
					"http://172.16.13.116:8080/user/logout",
					{}
				),
			]);
			...
			console.log("Logout Response Data 1:", data1);
			console.log("Logout Response Data 2:", data2);

			// 로컬 저장소에서 토큰 삭제 후 페이지 이동
			localStorage.removeItem("authToken");
			navigate("/");
		} catch (error) {
			console.error(
