		...
		const fetchData = async () => {
			try {
				...
				const response = await Promise.any([
					...
					axios.post(
						"http://172.16.13.116:8000/api/report/mwd"
					),
				]);
	...
	const handleSetGoal = async () => {
		...
		try {
			...
			const response = await axios.post(
				"http://127.0.0.1:8000/api/set-goal", // 서버에서 처리할 API 주소
				{
					month_goal: Number(monthGoal),
					week_goal: Number(weekGoal),
			...
			if (response.status === 200) {
				...
				console.error("목표 설정 실패");
			}
		} catch (error) {
			console.error("목표 설정 API 호출 실패:", error);
		}
		...
		<Container>
			...
			<Section>
				...
				<GoalWrapper>
					...
					>
						{formatCurrency(
							weeklySummary.withdraw_total || 0
						)}
						원
					</p>
				</GoalWrapper>
