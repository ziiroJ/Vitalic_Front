...
function GoalPage({ goal, setGoal }) {
  ...
  ///////**ㄹㅋ
  // 로컬 스토리지에서 목표 불러오기
  const loadGoalFromLocalStorage = () => {
  ...
  // 목표 저장 함수
  const saveGoalToLocalStorage = (goalData) => {
  ...
  useEffect(() => {
    loadGoalFromLocalStorage(); // 로컬 스토리지에서 목표 불러오기
    fetchUserData();
  ...
  const handleSetGoal = async () => {
    try {
      // 목표 예산을 서버에 전송
      await axios.post("http://localhost:8080/user/updateBudget", {
        dailyBudget: dayGoal,
        monthlyBudget: monthGoal,
        weeklyBudget: weekGoal,
      });

      // 목표 설정 후 goal 상태 업데이트ㄹㅋ
      const updatedGoal = {
        month: monthGoal,
        week: weekGoal,
        day: dayGoal,
      };
      setGoal(updatedGoal); // 목표 상태 업데이트ㄹㅋ
      saveGoalToLocalStorage(updatedGoal); // 로컬 스토리지에 목표 저장ㄹㅋ
      ...
      console.error("목표 예산을 설정하는 중 오류가 발생했습니다:", error);
    }
  };
  // 목표 대비 지출 비율 계산
      ...
      <Section>
        ...
        <GoalWrapper>
          ...
          <p style={{ color: "#888", fontSize: "0.8rem", margin: "0" }}>
            {formatCurrency(dailySummary.withdraw_total || 0)}원
          </p>
        </GoalWrapper>
        ...
        <GoalWrapper>
          ...
          {/* 현재 이번주 지출 금액 추가 */}
          <p style={{ color: "#888", fontSize: "0.8rem", margin: "0" }}>
            {formatCurrency(weeklySummary.withdraw_total || 0)}원
          </p>
        </GoalWrapper>
        ...
        <GoalWrapper>
          ...
          {/* 현재 이번 달 지출 금액 추가 */}
          <p style={{ color: "#888", fontSize: "0.8rem", margin: "0" }}>
            {formatCurrency(monthlySummary.withdraw_total || 0)}원
          </p>
        </GoalWrapper>
      </Section>
