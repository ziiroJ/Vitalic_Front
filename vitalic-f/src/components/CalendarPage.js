...
function CalendarPage() {
  ...
  const fetchData = async (year, month) => {
    try {
      ...
      // 이벤트 생성
      const newEvents = dailyData.flatMap(({ day, deposit, withdraw }) => {
      ...
      const groupedTransactions = dailyData.map(
        ({ day, deposits, withdrawals }) => {
          ...
          const combinedTransactions = [
            ...deposits.map((item) => ({ ...item, type: "입금" })),
            ...withdrawals.map((item) => ({ ...item, type: "출금" })),
          ];
          ...
          return { date, transactions: combinedTransactions };
        }
      );
  ...
  const handleDateSelect = async (date) => {
    ...
    try {
      ...
      const transactionDetails = [
        ...data.deposit_details.map((item) => ({ ...item, type: "입금" })),
        ...data.withdraw_details.map((item) => ({ ...item, type: "출금" })),
      ];

      transactionDetails.sort(
        (a, b) => new Date(b.tran_date_time) - new Date(a.tran_date_time)
      );
          ...
          <ExpenseListWrap>
            {selectedDateTransactions.map((item, index) => (
              <ExpenseList key={index}>
                <Dot isMinus={item.in_des.includes("이체") ? false : true} />
                <div
                  style={{
                    display: "flex",
                ...
                >
                  ...
                  <span
                    style={{
                      marginLeft: "auto",
                      color: item.type === "입금" ? "#17A1FA" : "inherit",
                    }}
                  >
                    {item.type === "입금"
                      ? item.tran_amt.toLocaleString() + "원" // 입금일 경우 금액 표시
